...
"""
Script generates parameter files for the NASA SnowEx AK IOP April snow melt campaign
...
def get_metadata(xl_file):
    ...
    # plotID = d['Unnamed: 0'][6] # can use if pit sheet gets updated
    plotID = filename.name.split('_')[0] #01T1, 02T1, etc. (Megan assigned in email, printed on readme.doc)
    plot_num = plotID[:2]
    date = d['Unnamed: 6'][1]
    time = d['Unnamed: 6'][4]
    zone = d['Unnamed: 17'][6]
...
if __name__ == "__main__":
    ...
    src_path = Path('/Users/mamason6/Documents/snowex/campaigns/AKIOP-23/april/cleaned_pits')
    des_basepath = Path('/Users/mamason6/Documents/snowex/core-datasets/ground/snow-pits/run-AKIOP23/April/outputs')
    des_basepath.mkdir(exist_ok=True)
    ...
    for i, filename in enumerate(sorted(src_path.rglob('*.xlsm'))): #01T1_20240401
        # print(filename.name)
        ...
        new_partial_file_path, flight_line = generate_new_filename(filename) # returns path and string # leaving but "flight name" = "site"
        ...
        new_filename = Path(campaign_prefix + filename.stem + '_pitSheet_' + version + '.xlsx') # upated here..filename.suffix
        ...
        # convert to standard time (-1hr if >= March 12, 2023) and resave file # THIS APPLIES TO APRIL DATA
        ...
        fname_density         = pitPath.joinpath(campaign_prefix + filename.stem + '_density_' + version +'.csv')
        fname_gapFilledDensity= pitPath.joinpath(campaign_prefix + filename.stem + '_gapFilled_density_' + version +'.csv')
        fname_LWC             = pitPath.joinpath(campaign_prefix + filename.stem + '_LWC_' + version +'.csv')
        fname_temperature     = pitPath.joinpath(campaign_prefix + filename.stem + '_temperature_' + version +'.csv')
        fname_stratigraphy    = pitPath.joinpath(campaign_prefix + filename.stem + '_stratigraphy_' + version +'.csv')
        fname_siteDetails     = pitPath.joinpath(campaign_prefix + filename.stem + '_siteDetails_' + version +'.csv')
